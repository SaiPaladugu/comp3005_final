...
def Q_3(conn, execution_time):
    ...
    SELECT 
        Shot.player_name,
        COUNT(Shot.event_id) AS number_of_first_time_shots
    FROM 
        Shot
    JOIN 
        Matches ON Shot.match_id = Matches.match_id
    ...
    WHERE 
        Competitions.competition_name = 'La Liga' 
        AND Competitions.season_name IN ('2020/2021', '2019/2020', '2018/2019')
...
def Q_5(conn, execution_time):
    ...
    query = """
    SELECT 
        Pass.recipient_name AS player_name,
        COUNT(Pass.event_id) AS number_of_times_as_recipient
    FROM 
        Pass
    JOIN 
        Matches ON Pass.match_id = Matches.match_id
    ...
    WHERE 
        ...
        AND Pass.recipient_name IS NOT NULL
    GROUP BY 
        Pass.recipient_name
    HAVING 
        COUNT(Pass.event_id) >= 1
    ORDER BY 
        number_of_times_as_recipient DESC;
    """
...
def Q_7(conn, execution_time):
    ...
    SELECT 
        Pass.player_name,
        COUNT(Pass.event_id) AS number_of_through_balls
    FROM 
        Pass
    JOIN 
        Matches ON Pass.match_id = Matches.match_id
...
def Q_9(conn, execution_time):
    ...
    SELECT 
        Dribble.player_name,
        COUNT(Dribble.event_id) AS number_of_successful_dribbles
    FROM 
        Dribble
    JOIN 
        Matches ON Dribble.match_id = Matches.match_id
    JOIN 
        Competitions ON Matches.competition_id = Competitions.competition_id 
        AND Matches.season_id = Competitions.season_id
    WHERE 
...
def Q_10(conn, execution_time):
    ...
    query = """
    SELECT 
        Dribbled_Past.player_name,
        COUNT(Dribbled_Past.event_id) AS number_of_times_dribbled_past
    FROM 
        Dribbled_Past
    JOIN 
        Matches ON Dribbled_Past.match_id = Matches.match_id
    JOIN 
        Competitions ON Matches.competition_id = Competitions.competition_id 
        AND Matches.season_id = Competitions.season_id
    WHERE 
